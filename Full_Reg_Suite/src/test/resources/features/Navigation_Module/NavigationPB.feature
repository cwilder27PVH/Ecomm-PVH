...
Scenario Outline: Verify department category links go to correct page
	...
	Examples:
	| brand  | dept 	    |	 links					     |
	| VH     | teams	    |	womens;mens                  |
	
 @id=3 @positive @parallel @navigation_module @brand=PB
Scenario Outline: Verify department category link breadcrumbs on cat pages revert back to department
	...
	Examples:
	| brand  |  dept 	|	 links						    			| 
	| VH     | 	kids	|	 footwear;girls swimwear;boys swimwear	    | 

@id=4 @positive @parallel @navigation_module @brand=PB
...
@id=5 @positive @parallel @navigation_module @brand=PB
...
@id=6 @positive @parallel @navigation_module @brand=PB
Scenario Outline: verify cart button brings you to the cart
	Given User access <brand> website
